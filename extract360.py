...
def handle_con(infile, fsize):
    ...
    txtfile = open_info_file(infile)
    if txtfile != None:
        print >> txtfile, "(Console specific) header (hex):",
...
def handle_fmim(infile, fsize):
    ...
    txtfile = open_info_file(infile)
    if txtfile != None:
        print >> txtfile, "Song:", strip_blanks(infile.read(0x200))
        print >> txtfile, "Album:", strip_blanks(infile.read(0x200))
        print >> txtfile, "Artist 1:", strip_blanks(infile.read(0x200))
        print >> txtfile, "Artist 2:", strip_blanks(infile.read(0x200))
        print >> txtfile, "Genre 1:", strip_blanks(infile.read(0x200))
        print >> txtfile, "Genre 2:", strip_blanks(infile.read(0x200))
        # 4 byte "size" info, followed by 248 0-bytes
        print >> txtfile, "Size:", struct.unpack(">L", infile.read(4))[0]
        zerobuf = infile.read(0xF8)
        if False in [x == '\0' for x in zerobuf]:
            print >> txtfile, "Not followed by 248 0-bytes:",
            for i in zerobuf:
                print >> txtfile, hex(ord(i)),
            print >> txtfile
        txtfile.close()
...
def handle_xuiz(infile, fsize):
    ...
    if txtfile != None:
        print >> txtfile, "Flags: ", xui_flags
        print >> txtfile, "Size: ", xui_len
        print >> txtfile, "? : ", xui_smth
        print >> txtfile, "Data pointer: ", xui_data_ptr
        print >> txtfile, "Directory entries: ", xui_num_de
    ...
    for dirent in xrange(xui_num_de):
        if txtfile != None:
            print >> txtfile, "\nEntry ", dirent
            (flen, fptr, fnlen) = struct.unpack(">LLB", buf[bufpos:bufpos + 9])
            bufpos += 9
            print >> txtfile, "File length: ", flen
            print >> txtfile, "File start: ", fptr
            print >> txtfile, "Filename length (UTF-16): ", fnlen
            filename = rm_nul(strip_blanks(buf[bufpos:bufpos + 2 * fnlen]))
            bufpos += 2 * fnlen
            print >> txtfile, "Filename: ", filename
        ...
        print >> outfile, infile.read(flen)
        outfile.close()
        for _ in xrange(dir_count):
            os.chdir("..")
